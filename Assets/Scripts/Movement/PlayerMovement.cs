...
public class PlayerMovement : MonoBehaviour
{
    ...
    //{
    //    // Поворачиваем модель по Y вслед за orientation (которая следует за камерой)
    //    playerModel.rotation = Quaternion.Slerp(
    //        playerModel.rotation,
    //        Quaternion.LookRotation(orientation.forward),
    //        Time.deltaTime * 15f // скорость поворота, подбери под себя
    //    );
    //}
    ...
    private void MovePlayer()
    {
        ...
        {
            // playerModel — ссылка на Transform модели
            playerModel.rotation = Quaternion.Slerp(
                playerModel.rotation,
